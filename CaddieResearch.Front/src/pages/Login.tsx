...
export default function Login() {
  ...
  useEffect(() => {
    // Quando o inProgress for 'none', significa que a página terminou de processar o redirecionamento
    // Se accounts.length for > 0, significa que tem um usuário logado na mochila!
    if (inProgress === "none" && accounts.length > 0) {
      setCarregando(true);

      // Pedimos para a Microsoft revelar o Token desse usuário
      instance.acquireTokenSilent({
        scopes: ["user.read", "email"],
        account: accounts[0]
      }).then((response) => {
        console.log("Token resgatado da mochila:", response.accessToken);
        enviarTokenMicrosoftParaOCSharp(response.accessToken);
      }).catch(error => {
        console.error("Erro silencioso da MS:", error);
        setCarregando(false);
      });
    }
  }, [inProgress, accounts, instance]);

  // Função que manda o token pescado para o Back-end
  const enviarTokenMicrosoftParaOCSharp = async (token: string) => {
    try {
      ...
      // Se der erro no C#, deslogamos da Microsoft para ele poder tentar de novo
  ...
  // O botão continua apenas redirecionando
